<script lang="ts">
	...
	const steps = [
		{ label: 'Оформление' },
		{ label: 'Анкета' },
		{ label: 'Код', description: 'Введите код из SMS' },
		{ label: 'Паспорт' },
	...
	function handleNext() {
		goto('/child-card/step4');
	}
</script>

<svelte:head>
	<title>Оформление детской карты - Шаг 3</title>
</svelte:head>

<div class="page">
	<BackButton onclick={() => history.back()} />

	<Stepper {steps} currentStep={3} title={`Заявка на выпуск \n Детской карты`} />
	...
	<div class="footer">
		<div class="footer-buttons">
			...
			<Button variant="primary" onClick={handleNext} disabled={!isCodeComplete}>
				Подтвердить
			</Button>
		</div>
	</div>
