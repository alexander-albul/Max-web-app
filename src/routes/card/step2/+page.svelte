<script lang="ts">
	...
	function handleGosuslugi() {
		localStorage.setItem('gosuslugi_data', JSON.stringify(gosuslugiData));
		lastName = gosuslugiData.lastName;
		firstName = gosuslugiData.firstName;
		middleName = gosuslugiData.middleName;
		birthDate = gosuslugiData.birthDate;
		phone = gosuslugiData.phone;
	...
	<div class="content">
			...
			<div class="gosuslugi-synced">
				<div class="gosuslugi-synced-icon">
					<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 29 28" fill="none">
						...
						<defs>
							<linearGradient id="gs_grad_synced" x1="14.2494" y1="0" x2="14.2494" y2="27.9997" gradientUnits="userSpaceOnUse">
		...
		<div class="form-section">
			...
			<div class="form-fields">
				<Input bind:value={lastName} label="Фамилия" required error={errors.lastName} onclick={() => { if (!lastName) lastName = autofill.lastName; }} />

				<Input bind:value={firstName} label="Имя" required error={errors.firstName} onclick={() => { if (!firstName) firstName = autofill.firstName; }} />

				<div class="field-with-checkbox">
					<Input bind:value={middleName} label="Отчество" disabled={noMiddleName} error={errors.middleName} onclick={() => { if (!middleName && !noMiddleName) middleName = autofill.middleName; }} />
					<Checkbox bind:checked={noMiddleName} label="Нет отчества" size="s" />
				</div>

				<DatePicker bind:value={birthDate} label="Дата рождения" required maxDate={new Date()} error={errors.birthDate} onclick={() => { if (!birthDate) birthDate = autofill.birthDate; }} />

				<Input bind:value={phone} label="Телефон" type="tel" required error={errors.phone} onclick={() => { if (!phone) phone = autofill.phone; }} />
			</div>
		</div>
