<script lang="ts">
	...
	const steps = [
		...
		{ label: 'Анкета' },
		{ label: 'Код' },
		{ label: 'Паспорт', description: 'Данные паспорта родителя' },
...
<svelte:head>
	<title>Оформление детской карты — Шаг 4</title>
</svelte:head>

<div class="page">
	<BackButton onclick={() => history.back()} />

	<Stepper {steps} currentStep={4} title={`Заявка на выпуск \n Детской карты`} />
...
<InputModal
	...
	onSave={(v) => { registrationAddress = v; if (sameAddress) actualAddress = v; }}
